...
class RedisDriver(Driver):
    ...
    def query(
        ...
        try:
            ...
            if "unknown command" in error_msg and read_only:
                ...
                return self.query(
                    cmd, graph_name, q, params, timeout, read_only=False, graph=graph
                )
    ...
    def commit(
        self, graph, items: list[Common] = None, batch_size: int = 50
    ) -> QueryResult | None:
        """
                ...
                try:
                    ...
                    if batch_size > 0:
                        for item in batch:
                            ...
                            item_result = self.query(
                                CMD.QUERY, graph.name, query, graph=graph
                            )
