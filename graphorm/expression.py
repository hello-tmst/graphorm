...
def add_query_param(value: Any, params: Dict[str, Any], dedupe: bool = False) -> str:
    """
    ...
    :param dedupe: If True, return existing key when value already in params
...
class BinaryExpression:
    ...
    def to_cypher(
        self, params: dict[str, Any] = None, alias_map: dict[Any, str] = None
    ) -> str:
        """
        ...
        if hasattr(self.left, "to_cypher"):
            ...
            if hasattr(self.left, "to_cypher") and hasattr(self.left, "node_class"):
                ...
                # Check if left has a label (Function or ArithmeticExpression with label)
                # In WHERE after WITH, we should use only the label, not the full expression
                if hasattr(self.left, "_label") and self.left._label:
                    # Use only the label in WHERE after WITH
                    left_str = self.left._label
                else:
        ...
        if self.operator == "IN":
            param_name = add_query_param(self.right, params)
            ...
            param_name = add_query_param(self.right, params, dedupe=False)
...
class AndExpression:
    ...
    def to_cypher(
        ...
        left_str = (
            self.left.to_cypher(params, alias_map)
            if hasattr(self.left, "to_cypher")
            else str(self.left)
        )
        right_str = (
            self.right.to_cypher(params, alias_map)
...
class Function:
    ...
    def desc(self) -> "OrderByExpression":
        """DESC ordering for function expression."""
        return OrderByExpression(self, "DESC")

    def asc(self) -> "OrderByExpression":
        """ASC ordering for function expression."""
        return OrderByExpression(self, "ASC")
    ...
    def to_cypher(
        ...
        for condition, value in self.when_then_pairs:
            ...
            if hasattr(value, "to_cypher"):
                ...
                param_name = add_query_param(value, params, dedupe=True)
        ...
        if self.else_value is not None:
            if hasattr(self.else_value, "to_cypher"):
                ...
                param_name = add_query_param(self.else_value, params, dedupe=True)
