...
"""
import pytest
from graphorm import Node, select, size, head, tail, last
...
def test_size_function(graph):
    ...
    stmt = select().match(PageAlias).where(size(PageAlias.tags) > 0).returns(PageAlias, size(PageAlias.tags).label("tag_count"))
...
def test_head_function(graph):
    ...
    # Query with head()
    PageAlias = Page.alias("p")
    stmt = select().match(PageAlias).returns(PageAlias, head(PageAlias.tags).label("first_tag"))
...
def test_tail_function(graph):
    ...
    # Query with tail()
    PageAlias = Page.alias("p")
    stmt = select().match(PageAlias).returns(PageAlias, tail(PageAlias.tags).label("rest_tags"))
...
def test_last_function(graph):
    ...
    # Query with last()
    PageAlias = Page.alias("p")
    stmt = select().match(PageAlias).returns(PageAlias, last(PageAlias.tags).label("last_tag"))
