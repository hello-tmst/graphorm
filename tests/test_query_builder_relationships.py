...
def test_match_relationship_pattern(graph):
    ...
    stmt = select().match((Page.alias("a"), Linked.alias("r"), Page.alias("b")))
    cypher = stmt.to_cypher()
...
def test_match_relationship_with_where(graph):
    ...
    stmt = (
        select().match((PageA, Linked.alias("r"), PageB)).where(PageA.path == "/home")
    )
    cypher = stmt.to_cypher()
...
def test_match_relationship_with_returns(graph):
    ...
    stmt = select().match((PageA, LinkedR, PageB)).returns(PageA, LinkedR, PageB)
    cypher = stmt.to_cypher()
...
def test_match_multiple_patterns(graph):
    ...
    stmt = select().match(
        PageA, (PageA, Linked.alias("r1"), PageB), (PageB, Linked.alias("r2"), PageC)
    )
    cypher = stmt.to_cypher()
...
def test_match_relationship_with_property_access(graph):
    ...
    stmt = (
        select()
        .match((PageA, LinkedR, PageB))
        .where((PageA.path == "/home") & (PageB.title.contains("About")))
    )
    cypher = stmt.to_cypher()
...
def test_match_relationship_execution(graph):
    ...
    stmt = select().match((PageA, Linked.alias("r"), PageB)).returns(PageA, PageB)
