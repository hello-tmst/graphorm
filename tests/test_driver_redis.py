...
def test_redis_driver_connection_error(mock_redis):
    ...
    mock_redis_instance.execute_command.side_effect = redis.exceptions.ConnectionError("Connection failed")
...
def test_redis_driver_timeout_error(mock_redis):
    ...
    mock_redis_instance = Mock()
    mock_redis_instance.execute_command.side_effect = redis.exceptions.TimeoutError("Query timeout")
    mock_redis.return_value = mock_redis_instance
...
def test_redis_driver_response_error(mock_redis):
    ...
    mock_redis_instance = Mock()
    mock_redis_instance.execute_command.side_effect = redis.exceptions.ResponseError("Query syntax error")
    mock_redis.return_value = mock_redis_instance
...
def test_redis_driver_timeout_validation(mock_redis):
    ...
    with pytest.raises(QueryExecutionError, match="Timeout argument must be a positive integer"):
